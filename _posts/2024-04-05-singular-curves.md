...
The Weierstrass normal form of elliptic curves (in fields with characteristic != 2 and != 3)
$$ y^2 = x^3 + ax + b
...
1. $$ y^2 = x^3
   where using mapping $$ (x, y) \rightarrow x/y
   that gives an isomorphism to additive group (with same p order) where discrete logarithm is trivial.

2. $$ y^2 = x^2(x + c)
   where using mapping $$ (x, y) \rightarrow (y + xc)/(y - xc)
   that gives an isomorphism to multiplicative group (with p^2 order) where discrete logarithm problem is easy to solve
   for default elliptic key sizes.

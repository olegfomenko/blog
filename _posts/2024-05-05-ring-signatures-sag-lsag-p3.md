...
{i,j}\}$$ for $$i \in \{1,2,...n\}$$ and $$j \in \{1,2,...m\}$$ $ where we know the private keys $$k_{x,j}$$ of the key
with secret position $$x$$ and corresponding public keys $$K_{x,j}$$ for $$j \in \{1,2,...m\}$$. The MLSAG protocol has
...
3. For $$i \in \{1,2,...n\} \setminus \{x\}$$ $$j \in \{1,2,...m\}$$ generate random $$r_{i,j}$$.
4. Put $$c_{x+1} = H(m, [a_1\cdot G], [a_1\cdot H_p(K_{x,1})], [a_2\cdot G], [a_2\cdot H_p(K_{x,2})],...)$$.
5. Then, for every $$i = x+1, x+2, ..., d, 1, 2, ..., x-1$$ (replacing $$d + 1 \rightarrow 1$$) calculate $$c_{i+1} = H(
...
prime order of the big subgroup of our elliptic curve. Then, for the every $$i = 1, 2, ..., d$$ (replacing $$d + 1
...
Finally, if two different signatures contains same value $$\hat{K}_t$$ for some $$t$$ then these signatures are linked by
same signer.

---
title: Intro to Lightning Network (Pt.1)
aside:
  toc: false
tags:
  - ln
...
> May thanks to Nazarii Shcherbak for his kind review.


I really am a big fan of the Layer-2 solutions and their approaches. IMHO, Layer-2 solutions are a state-of-the-art of
the overall blockchain technology. Their ideas lies on the most novel cryptographic primitives, which contribute to a
...
It is known that Bitcoin has an average block time of 10 minutes, while the required number of blocks for finalization
...
to the network: during the $T$ hours, the second party can punish the cheater by taking all tokens from the channel.
...
Above, we described the simplest way for two parties to exchange tokens with each other. Nevertheless, such an approach
...
knowledge of the pre-image by X or after some time by Y. More precisely, imagine Alice wants to send some tokens to Bob
...
8. Alice sends $\texttt{revoke\_and\_ack}$ message to Bob assuming the final acceptance of $n+1$ state and revoking of
   ...
   Unfortunately, the current form of state commitments is inconvenient for Bob, because while he owns the tokens in
   HTLC,
   he can not use them in transfers. So, Alice and Bob have to update commitment transactions by removing the HTLCs and
   changing the corresponding balances in $\texttt{to\_local}$ and $\texttt{to\_remove}$ of both parties.
...
- Bob reveals the pre-image $i$ to claim tokens from Dave (Bob executes 9-13 with Dave).
- Dave now knows $i$ and can claim $A$ tokens from Charlie (Dave executes 9-13 with Charlie).
- Charlie now knows $i$ and can claim tokens from Alice (Charlie executes 9-13 with Alice).

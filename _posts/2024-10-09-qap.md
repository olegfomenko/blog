...
selects $$\alpha$$ and share
$$g^{\alpha\cdot\tau^i}$$ with the prover, and the prover adds to the response the shifted polynomial encryption $$g^{p(
\alpha\tau)}$$. Then, the verifier adds check for $$g^{p(\alpha\tau)} = (g^{p(\tau)})^\alpha$$. While the raw value of
$$\alpha$$ is not accessible by the prover we reach the soundness property.

Unfortunately (x2), we still do not receive zero-knowledge. Because the $$\tau$$ and $$\alpha$$ seems to be generated

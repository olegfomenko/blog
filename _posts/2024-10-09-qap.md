...
\mathbf{w}\rangle$$. Where the one constraint is represented by the public $$\mathbf{a}, \mathbf{b}, \mathbf{c}$$
vectors. In the example of more complex circuit, we've received 4 constrains, so we will have 4 tuples of vectors - each
...
At the beginning, we should return to our note in previous post, where I've mentioned that we can combine all tuples
...
If this evaluation can be equal to zero - this proving system does not make any sense, but when the probability of such
...
Ok, this protocol follows the correctness and soundness properties, but how about the zero-knowledge? The proof values
queried by the verifier opens lots of information about the witness, because the interpolated polynomials over the
witness
...
polynomial $$t(x)$$. Then, we consider the following protocol: verifier shares a _powers of tau_ $$g^{\tau^i}$$ for some
generator $$g$$ - the securely generated powers of randomly selected $$\tau$$ in such way that nobody knows the $$\tau$$
itself. Then, prover calculates the polynomial encryption for $$p(x)$$ in the following way: $$g^{p(\tau)} = \sum (
g^{\tau^i})^a_i$$, where $$a_i$$ is a coefficients of $$p(x)$$. The same is done for $$h(x\tau) = \frac{p(\tau)}{t(
\tau)}$$. The verifier checks that $$g^{p(\tau)} = (g^{h(\tau)})^{t(\tau)}$$.
